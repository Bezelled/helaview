...
import { hotelRegistrationKeys, emailRegExp, passwordRegExp, saltRounds, AccountType, districts, upload } from '../../config/globals.js';
...
export default async function addRoute(router: Router): Promise<void>{
    ...
    router.post('/hotels/register',
        ...
        async(req: Request, res: Response) => {
            ...
            console.log('FILES', req.file ? req.file : 'No file');

            if (!req.files)
                return res.status(400).json({ error: 'Please upload some images of your hotel (1 - 5).' });
